...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, normalization=nn.BatchNorm):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.conv1 = nn.Sequential(nn.Conv(in_channels, out_channels, kernel_size, padding=1), normalization(out_channels), nn.ReLU())
        self.conv2 = nn.Sequential(nn.Conv(out_channels, out_channels, kernel_size, padding=1), normalization(out_channels), nn.ReLU())
...
class DownsampleBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_block=BasicBlock):
        ...
        self.down = nn.Pool(2, stride=2, op='mean')

    def execute(self, inputs, mask=None):
...
class UpsampleBlock(nn.Module):
    ...
    def execute(self, inputs1, inputs2):
        in1_up = self.up(inputs1)
        if ((inputs2.shape[2] != in1_up.shape[2]) or (inputs2.shape[3] != in1_up.shape[3])):
            ...
            diff2 = ((inputs2.shape[2] - in1_up.shape[2]) // 2)
            diff3 = ((inputs2.shape[3] - in1_up.shape[3]) // 2)
            inputs2_ = inputs2[:, :, diff2: diff2 + in1_up.size(2), diff3: diff3 + in1_up.size(3)]
        else:
            inputs2_ = inputs2
        output = self.conv(jt.contrib.concat([in1_up, inputs2_], dim=1))
        return output

class UNet(nn.Module):

    def __init__(self, num_input_channels=3, num_output_channels=3, feature_scale=4, upsample_mode='bilinear', last_act='sigmoid', conv_block='gated'):
        super().__init__()
