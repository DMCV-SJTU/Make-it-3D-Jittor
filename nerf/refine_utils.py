...
def multidepth2point_mask(allD, alphamask, allimg, cam, c2w, cano_v, cano_c2w, cano_D, H, W, radius, ppp, outputdir,
    ...
    K_tensor = jt.array(cam).to(device).float()
    ...
    for i in pbar:
        if i > -1:
            ...
            imageio.imwrite(outputdir + '/render_%s_mask.png' % (i),
                            np.array(cano_mask[0].permute(1, 2, 0).cpu().numpy() * 255, dtype=np.uint8))
...
def rand_poses(index, device, radius_range=[1, 1.5], theta_range=[0, 100], phi_range=[0, 360]):
    ...
    if index % 4 == 0:
        ...
        phis = jt.ones(size) * (phi_range[1] - phi_range[0]) / 2 + phi_range[0]
        is_front = True
    else:
...
def fix_poses(index, device, radius_range=[1, 1.5], theta_range=[0, 100], phi_range=[0, 360]):
    ...
    if index % 4 == 0:
        ...
        if phi_range[1] <= np.deg2rad(240.0) and phi_range[0] >= np.deg2rad(120.0):
            ...
            rand = random.random()
            if rand > 0.85:
                phis = jt.rand(size) * (phi_range[1] - np.deg2rad(315.0)) + np.deg2rad(315.0)
            elif rand > 0.7:
                phis = jt.rand(size) * (np.deg2rad(45.0) - phi_range[0]) + phi_range[0]
            elif rand > 0.5:
                phis = jt.rand(size) * (np.deg2rad(315.0) - np.deg2rad(240.0)) + np.deg2rad(240.0)
            elif rand > 0.3:
...
def render_point(points_xyz_org, points_color, H, W, K, world2cam, image_size, radius, ppp, bg_feat=None,
    ...
    points_idx, z_buf, dist = rasterize_points.rasterize_points(
        pts3D,
        image_size,
        radius,
        ppp
