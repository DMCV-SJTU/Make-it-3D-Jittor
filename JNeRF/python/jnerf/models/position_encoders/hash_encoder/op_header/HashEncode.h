...
#pragma unroll
		for (uint32_t feature = 0; feature < N_FEATURES_PER_LEVEL; ++feature)
		{
			...
			if (fabsf(data) < quantize_threshold) // useless since quantize_threshold=0 -WXZ
				data = 0.f;
			((T *)&result)[feature] += (T)(weight * data);
		}
	}

    //TODO(WXZ): Analyse this? I think it may write into the wrong dist
//	encoded_positions[i + level * num_elements] = result;
    //Here is the tcnn version
    #pragma unroll
	for (uint32_t f = 0; f < N_FEATURES_PER_LEVEL; ++f) {
...
//        auto m_encoded_pos=(vector_t<grad_t,N_FEATURES_PER_LEVEL>*)m_encoded_positions2;
		// WXZ NOTE: i + (level * N_FEATURES_PER_LEVEL + f) * num_elements = 60694692
		// => i=1974436  level=14  f=0
...
//                if (hashmap_offset_table[level] * N_FEATURES_PER_LEVEL + index + f == 4880){
//
//                    printf("max %.15f, %.15f %.15f, %d, %d\n", grid_gradient[index + f], grad[f], weight, i, feature);
//
//                }
//                else if(hashmap_offset_table[level] * N_FEATURES_PER_LEVEL + index + f == 3826){
//                    printf("min %.15f, %.15f %.15f, %d, %d\n", grid_gradient[index + f], grad[f], weight, i, feature);
//                }
////
